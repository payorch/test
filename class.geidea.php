...
?>
    ...
    #[NoReturn] public function init_payment()
    {
        //if ( (isset($_POST['fields']) && !empty($_POST['fields'])) ) {
            if ( wp_verify_nonce(sanitize_text_field(wp_unslash(isset($_POST['fields'])) ))) {
    ...
    public function process_admin_options(): bool {
        ...
        if (sanitize_text_field(wp_unslash(isset($_FILES['woocommerce_geidea_logo']))) && sanitize_text_field(wp_unslash(isset($_FILES['woocommerce_geidea_logo']['size']))) > 0) {
            $field = 'logo';
            $upload_errors = $this->upload_logo(sanitize_text_field(wp_unslash($_FILES['woocommerce_geidea_logo'])), $field);
            $this->errors = array_merge($this->errors, $upload_errors);
        }

        if (sanitize_text_field(wp_unslash(isset($_FILES['woocommerce_geidea_checkout_icon']))) && sanitize_text_field(wp_unslash(isset($_FILES['woocommerce_geidea_checkout_icon']['size']))) > 0) {
            $field = 'checkout_icon';
            $upload_errors = $this->upload_logo(sanitize_text_field(wp_unslash($_FILES['woocommerce_geidea_checkout_icon'])), $field);
            $this->errors = array_merge($this->errors, $upload_errors);
        }
